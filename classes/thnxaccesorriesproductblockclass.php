...
class thnxaccesorriesproductblockclass extends ObjectModel
{
	public $id;
	public $id_thnxaccesorriesproductblock;
	public $position;
	public $hook;
	public $content;
	public $title;
	...
	public $pages;
	private static $module_name = 'thnxaccesorriesproductblock';
	private static $tablename = 'thnxaccesorriesproductblock';
	private static $classname = 'thnxaccesorriesproductblockclass';
	public static $definition = array(
		'table' => 'thnxaccesorriesproductblock',
		'primary' => 'id_thnxaccesorriesproductblock',
		'multilang' => true,
		'fields' => array(
    ...
    public static function GetProductBlock($hook = NULL,$params = NULL)
    {
        if($hook == NULL)
            return false;
        if($params == NULL)
        ...
        if(isset($allproducts) && !empty($allproducts)){
            ...
            foreach ($allproducts as $allprds){
                ...
                if($module_name::PageException($allprds['pages'])){
                    ...
                    $product_item = self::GetaccesorriesProducts($params,$count_prd);
                    // $allprds['products'] = self::GetProductProperties($product_type,$product_item,$count_prd,$order_by,$order_way);
                    // start 1.7
    ...
    public static function GetaccesorriesProducts($params = NULL,$count_prd = 8){
    	if($params == NULL)
    		return false;
    		$id_product = (int)$params['id_product'];
			$category_products = self::getAccessoriesblock($id_product,$count_prd); 
			if(isset($category_products) && !empty($category_products)){
				foreach ($category_products as $category_product) {					
					$results[] = 'prd_'.$category_product['id_product'];
				}
			}
    ...
    public static function getAccessoriesblock($id_product = null,$count_prd = 8)
    {
    	if($id_product == null)
    		return false;
        return Db::getInstance()->executeS('
			...
			'.Shop::addSqlAssociation('product', 'p').' WHERE p.active = 1 and `id_product_1` = '.(int)$id_product.' LIMIT '.(int)$count_prd );
    }
