<?php
include_once(_PS_MODULE_DIR_.'thnxaccesorriesproductblock/classes/thnxaccesorriesproductblockclass.php');
class thnxaccesorriesproductblock extends Module
{
	private $_html = '';
	private $_hooks = '';
	public static $inlinejs = array();
	public static $module_name = 'thnxaccesorriesproductblock';
	public static $tablename = 'thnxaccesorriesproductblock';
	public static $classname = 'thnxaccesorriesproductblockclass';
	public $css_files = array(
		array(
			'key' => 'thnxaccesorriesproductblock_css',
			'src' => 'thnxaccesorriesproductblock.css',
			'priority' => 50,
			'media' => 'all',
			'load_theme' => false,
		),
	...
	public $js_files = array(
		array(
			'key' => 'thnxaccesorriesproductblock_js',
			'src' => 'thnxaccesorriesproductblock.js',
			'priority' => 50,
	...
	{
		$this->name = 'thnxaccesorriesproductblock';
		...
		$this->displayName = $this->l('Platinum Theme Accesorries Products Block');
		$this->description = $this->l('Platinum Theme Accesorries Products Block Modules.');
		...
		if((isset($this->context->controller->controller_type)) && ($this->context->controller->controller_type == 'front' || $this->context->controller->controller_type == 'modulefront')){
			global $smarty;
			smartyRegisterFunction($smarty, 'block', 'thnxaccesorriesproductblock_js', array('thnxaccesorriesproductblock', 'thnxaccesorriesproductblock_js'));
	...
	private function GetHookarray(){
		...
		if(isset($this->_hooks['display']) && !empty($this->_hooks['display'])){
			...
			foreach ($this->_hooks['display'] as $_hook) {
				$results[$i]['id'] = $_hook;
				if($_hook == 'displayProductExtraContent'){
					$results[$i]['name'] = 'Product Tab';
				}else{
	...
	public static function PageException($exceptions = NULL)
	{
		...
		$page_name = $context->controller->php_self;
		...
		if($page_name == 'category'){
			...
			$prd_cat_sql_key = md5($prd_cat_sql);

	    	if(!Cache::isStored($prd_cat_sql_key)){
	    		$prd_catresults_cache = Db::getInstance()->executeS($prd_cat_sql);
				Cache::store($prd_cat_sql_key,$prd_catresults_cache);
	    	}
			$prd_catresults = Cache::retrieve($prd_cat_sql_key);
			...
			$prd_man_sql_key = md5($prd_man_sql);
			if(!Cache::isStored($prd_man_sql_key)){
				$prd_manresults_cache = Db::getInstance()->executeS($prd_man_sql);
				Cache::store($prd_man_sql_key,$prd_manresults_cache);
			}
			$prd_manresults = Cache::retrieve($prd_man_sql_key);
			...
			$prd_sup_sql_key = md5($prd_sup_sql);
			if(!Cache::isStored($prd_sup_sql_key)){
				$prd_supresults_cache = Db::getInstance()->executeS($prd_sup_sql);
				Cache::store($prd_sup_sql_key,$prd_supresults_cache);
			}
			$prd_supresults = Cache::retrieve($prd_sup_sql_key);
	...
	public function getblock_lists($active = null)
	{
        ...
        $sql_key = md5($sql);
        if(!Cache::isStored($sql_key)){
        	$results_cache = Db::getInstance()->executeS($sql);
        	Cache::store($sql_key,$results_cache);
        }
        $results = Cache::retrieve($sql_key);
	...
	public function BlockExists($id_i)
	{
		...
		$req_key = md5($req);
    	if(!Cache::isStored($req_key)){
    		$row_cache = Db::getInstance()->getRow($req);
			Cache::store($req_key,$row_cache);
    	}
		$row = Cache::retrieve($req_key);
		return ($row);
	}
	...
	public static function AllPageExceptions()
	{
	    ...
	    $sql = 'SELECT p.`id_product`, pl.`name`
	   	...
	   	$sql_key = md5($sql);
	   	if(!Cache::isStored($sql_key)){
	    	$products_cache = Db::getInstance()->executeS($sql);
	   		Cache::store($sql_key,$products_cache);
	   	}
	   	$products = Cache::retrieve($sql_key);
	...
	public static function thnxaccesorriesproductblock_js($params, $content, &$smarty)
	{
	...
	public function hookexecute($params = null,$hook,$istab = false)
	{
		if($params == null)
			return false;
		$html = '';
		...
		$query_values = $classname::GetProductBlock($hook,$params);
		if(isset($query_values) && !empty($query_values)){
			foreach ($query_values as $query => $vale){
				...
				if($istab == false){
					...
					$html .= $this->display(__FILE__,'views/templates/front/layout/'.$layout.'.tpl');
				}else{
					$html .= $this->display(__FILE__,'views/templates/front/'.$istab.'.tpl');
				}
			}
		}
		return $html;
	...
	public function hookdisplayProductExtraContent($params)
	{
		...
		$classname = self::$classname;
		$mod_name = self::$module_name;
		$query_values = $classname::GetProductBlock('displayProductExtraContent',$param);
		if(isset($query_values) && !empty($query_values)){
			foreach ($query_values as $query => $vale){
				$this->context->smarty->assign(array($mod_name => $vale));
				$html = $this->fetch('module:'.$this->name.'/views/templates/front/tabcontent.tpl');
				$ProductExtraContent = new PrestaShop\PrestaShop\Core\Product\ProductExtraContent;
		        if(isset($vale['title']) && !empty($vale['title'])){
		        	$ProductExtraContent->setTitle($vale['title']);
		        }
		        if(isset($html) && !empty($html)){
		        	$ProductExtraContent->setContent($html);
		        }
		        $product_tabs[] = $ProductExtraContent;
			}
		}
		return $product_tabs;
	}
	public function hookdisplayLeftColumn($params)
	{
		...
		$param = array();
		$param['id_product'] = $id_product;
		return $this->hookexecute($param,'displayLeftColumn');
	}
	public function hookdisplayRightColumn($params)
	{
		...
		$param = array();
		$param['id_product'] = $id_product;
		return $this->hookexecute($param,'displayRightColumn');
	}
	public function hookdisplayRightColumnProduct($params)
	{
		...
		$param = array();
		$param['id_product'] = $id_product;
		return $this->hookexecute($param,'displayRightColumnProduct');
	}
	public function hookdisplayFooterProduct($params)
	{
		...
		$param = array();
		$param['id_product'] = $id_product;
		return $this->hookexecute($param,'displayFooterProduct');
	}
}